...
def main(*args):
    ...
    url = (
        uri + "fmc_config/v1/domain/" + domain_uuid + "/deviceclusters/ftddevicecluster"
    )
    headers = {"X-auth-access-token": token}
    parameters = {"expanded": "true"}
    response = requests.request(
        "GET", url, headers=headers, params=parameters, verify=False
    )
    ...
    url = (
        uri
        + "fmc_config/v1/domain/"
        + domain_uuid
        + "/policy/accesspolicies/"
        + acp_id
        + "/operational/hitcounts"
    )
    headers = {"X-auth-access-token": token}
    parameters = {
    ...
    response = requests.request(
        "PUT", url, headers=headers, params=parameters, verify=False
    )
    time.sleep(29)
    ...
    while limit == True:
        ...
        if int(paging.get("limit")) != 100:
            limit = False
        # Extract information from response
        row = []
        for rule in rules:
            row.append(rule.get("metadata").get("ruleIndex"))
